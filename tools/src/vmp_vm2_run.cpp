...
Options parse_args(int argc, char** argv) {
  ...
  if (options.module_path.empty()) throw std::runtime_error("module path is required");
...
int main(int argc, char** argv) {
  try {
    ...
    const auto module = vmp::runtime::vm2::Vm2Module::load_from_file(options.module_path);
    vmp::runtime::vm2::Vm2Context context(module);
    for (int i = 0; i < static_cast<int>(options.args.size()) && i < 8; ++i) {
      context.r[static_cast<std::size_t>(i)] = std::strtoull(options.args[static_cast<std::size_t>(i)].c_str(), nullptr, 0);
    }
    if (options.args.size() > 8) {
      const auto spill_count = options.args.size() - 8;
      const auto spill_base = context.allocate_spill(spill_count * sizeof(std::uint64_t), "cli spill");
      for (std::size_t i = 0; i < spill_count; ++i) {
        context.write_memory<std::uint64_t>(spill_base + static_cast<std::uint64_t>(i * sizeof(std::uint64_t)),
                                            std::strtoull(options.args[8 + i].c_str(), nullptr, 0));
      }
    }
    ...
    if (!options.audit_path.empty()) {
      ...
      vmp::runtime::jit::Vm2Jit::instance().set_audit_writer(writer.get());
    }
    auto& runtime_state = vmp::runtime::state::RuntimeState::instance();
    runtime_state.shutdown();
    runtime_state.init_once(writer.get(), {VMP_PLATFORM_STR, "vmp-vm2-run", false});
    ...
    if (!options.string_pool_path.empty() || !options.string_idx_path.empty()) {
      ...
      auto key = std::make_shared<vmp::runtime::strings::KeyContext>(vmp::tools::strings_tool::key_from_env(options.key_env), salt);
      context.key_context = key;
      context.string_pool = std::make_shared<vmp::runtime::strings::StringPool>(
          vmp::tools::strings_tool::read_binary(options.string_pool_path), index,
          vmp::runtime::strings::KeyContext(vmp::tools::strings_tool::key_from_env(options.key_env), salt));
    ...
    if (!options.profile_path.empty()) {
      ...
      input.modules[module.id()].current_budget_bytes = vmp::runtime::jit::Vm2Jit::instance().module_cache_budget_bytes();
      vmp::runtime::state::SchedulerBindings bindings;
      bindings.vm2_modules[module.id()] = {&module, &context};
    ...
    for (const auto& [pc, hits] : module.function_hit_counters) {
      for (std::uint64_t i = 0; i < hits; ++i) runtime_state.hot_recorder().record_function_entry(module.id(), pc);
      const auto stats = vmp::runtime::jit::Vm2Jit::instance().entry_stats(module.id(), pc);
    ...
    std::cout << "ret_int=" << result.ret_int << " ret_float=" << result.ret_float
              << " ret_vec_lo=" << result.ret_vec.u64.lo << " ret_vec_hi=" << result.ret_vec.u64.hi << '\n';
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "vmp-vm2-run failed: " << ex.what() << '\n';
    return 1;
  }
}
