...
common::LiftedFunction Arm64Lifter::lift(const common::FunctionView& view) const {
  ...
  for(std::size_t off=0; off<view.code.size(); off+=4){ auto w=static_cast<std::uint32_t>(common::read_integer(view,off,4)); Decoded d; d.offset=off; d.word=w; d.rd=w&0x1Fu; d.rn=(w>>5)&0x1Fu; d.rm=(w>>16)&0x1Fu; d.is64=((w>>31)&1u)!=0u; d.pc_relative_target = decode_instruction(le32(w), view.base_addr + off).pc_relative_target;
    ...
    else if((w & 0x3B000000u)==0x18000000u){ d.kind="ldr_lit"; d.target=d.pc_relative_target.has_value()?d.pc_relative_target->computed_absolute:0; d.is64=(((w >> 30) & 0x3u) == 1u); }
