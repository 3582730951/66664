...
- `arch/common/pc_relative.h`：`PcRelativeTarget` / `Label` / `encode_rel*` / `encode_pc_page`。
...
- PC-relative：
  - decoder IR 统一追加 `std::optional<PcRelativeTarget> pc_relative_target`
  - `source_pc` 记录 ISA 语义上的位移基准 PC，`displacement` 统一按 byte 保存
  - 控制流优先 lowering 到 VM label；地址物化/字面量读取在本轮保守地做“内部 label 或 lift-time constant fold”
  - 真正跨 basic-block 的 label 解析阶段在 subtask 21 继续扩展
- ABI 入口映射：
